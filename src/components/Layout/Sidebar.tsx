...
export default function Sidebar({ activeTab, setActiveTab, isMobile, onClose }) {
  ...
  let containerClass = "h-full flex flex-col";
  if (isMobile) {
    containerClass += " w-64 bg-white/95 backdrop-blur-xl";
  } else {
    containerClass += " w-72 fixed left-0 top-0 glass";
  }

  return (
    <div className={containerClass} style={{ borderRight: '1px solid rgba(45, 154, 138, 0.1)' }}>
      <div className="p-6">
      ...
      <div className="p-4 border-t" style={{ borderColor: 'rgba(45, 154, 138, 0.1)' }}>
        <div className="flex items-center space-x-3 mb-3 px-2">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate" style={{ color: '#0d312c' }}>{currentStudent?.studentName || 'Student'}</p>
            <p className="text-xs truncate" style={{ color: '#66c3b7' }}>{currentStudent?.email}</p>
          </div>
        </div>
        <button 
          onClick={logout}
